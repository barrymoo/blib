...
template <typename A> std::vector<A> sort(std::vector<A> &a) {
  std::vector<A> _a(a.size());
  std::partial_sort_copy(a.begin(), a.end(), _a.begin(), _a.end());
  return _a;
};

template <typename T> void sort_inplace(std::vector<T> &t) {
...
template <typename T>
void for_each(const std::vector<T> &t, std::function<void(const T &)> f) {
